...
class EnvKeyProvider:
    """``MAILBOX_API_MASTER_KEY`` holds the recovery key. Read only."""
    ...
    def store(self, key: bytes) -> None:
        raise KeyProviderError(
            "the env key provider cannot store a key: set MAILBOX_API_MASTER_KEY "
            "to the recovery key instead"
        )

    def describe(self) -> str:
        return "the environment variable MAILBOX_API_MASTER_KEY"
