...
from benethos_mailbox_api.data.secrets import FileKeyProvider, cipher, decode_recovery
from benethos_mailbox_api.data.secrets.backup import (
    MAGIC,
    BackupError,
    Manifest,
    create_backup,
...
def machine(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setenv("MAILBOX_API_KEY_FILE", str(tmp_path / "secret" / "master.key"))
    return tmp_path
...
def test_backup_verify_restore_commands(
    ...
    monkeypatch.setenv("MAILBOX_API_DATA_DIR", str(machine / "new-data"))
    new_key_file = machine / "new-secret" / "master.key"
    monkeypatch.setenv("MAILBOX_API_KEY_FILE", str(new_key_file))
...
def test_backup_command_errors(
    ...
    assert main(["backup", str(machine / "x.bak")]) == 1
    assert "MAILBOX_API_STORAGE=sqlite" in capsys.readouterr().err
