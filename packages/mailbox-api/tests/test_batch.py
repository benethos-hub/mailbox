...
from benethos_mailbox_api.errors import MailboxApiError, NotFoundError
...
async def test_one_store_per_set_of_changes(server: FakeMailBox) -> None:  # noqa: F811
    ...
    assert all(
        not isinstance(r, MailboxApiError) and r.starred for r in results.values()
    )
...
async def test_one_move_for_many(server: FakeMailBox) -> None:  # noqa: F811
    ...
    new = {r.id for r in results.values() if not isinstance(r, MailboxApiError)}
...
async def test_unknown_ones_fail_alone(server: FakeMailBox) -> None:  # noqa: F811
    ...
    assert not isinstance(results[inbox(1)], MailboxApiError)
...
async def test_a_batch_into_the_trash(server: FakeMailBox) -> None:  # noqa: F811
    ...
    assert all(
        ...
        if not isinstance(r, MailboxApiError)
    )
