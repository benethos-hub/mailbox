...
from benethos_mailbox_api.data.discovery import (
    Finding,
    Query,
    SafeFetcher,
    default_sources,
...
from benethos_mailbox_api.domain import discovery as discovery_module
from benethos_mailbox_api.domain.access import Access
from benethos_mailbox_api.domain.discovery import DiscoveryService
...
def test_settings_switch_ispdb_off(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MAILBOX_API_DISCOVERY_ISPDB", "false")
    monkeypatch.setenv(
        "MAILBOX_API_DISCOVERY_INTERNAL_HOSTS", '["mail.intern.example"]'
    )
    settings = Settings()
