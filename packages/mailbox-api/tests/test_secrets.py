...
from benethos_mailbox_api.data.secrets import (
    CredentialVault,
    EnvKeyProvider,
    FileKeyProvider,
    KeyProviderError,
...
def test_env_provider() -> None:
    ...
    assert "MAILBOX_API_MASTER_KEY" in EnvKeyProvider(None).describe()
...
def test_stored_encrypted_and_never_returned(
    ...
    monkeypatch.setenv("MAILBOX_API_MASTER_KEY", master)
